...
![**图 8.8** $N \times N$ 的 $\mathbf{QK^T}$ 矩阵，展示了如何通过一次矩阵乘法同时计算所有 $\mathbf{q}_i \cdot \mathbf{k}_j$ 的相似度。](/images/speech-and-language-processing/slp-fig-8-8.png)
...
![**图 8.9** $N \times N$ 的 $\mathbf{QK}^\top$ 矩阵，显示了所有 $\mathbf{q}_i \cdot \mathbf{k}_j$ 的值，其中比较矩阵的上三角部分已被置零（实际设为 $-\infty$，经 softmax 后变为 0）。](/images/speech-and-language-processing/slp-fig-8-9.png)

图 8.10 则以示意图形式展示了单个注意力头在矩阵形式下的完整并行计算流程。

![**图 8.10** 单个注意力头的并行化注意力计算示意图。第一行展示了 $\mathbf{Q}$、$\mathbf{K}$ 和 $\mathbf{V}$ 矩阵的计算；第二行展示了 $\mathbf{QK}^\top$ 的计算、掩码操作（图中未显示缩放和 softmax 步骤），以及对值向量进行加权求和以得到最终的注意力输出向量。](/images/speech-and-language-processing/slp-fig-8-10.png)
