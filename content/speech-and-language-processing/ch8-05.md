...
![**图 8.14** 语言建模头部：位于 Transformer 顶部的结构，将最后一层对词元 $N$ 的输出嵌入 $h^L_N$ 映射为词汇表 $V$ 上的概率分布。](/images/speech-and-language-processing/slp-fig-8-14.png)
...
![**图 8.15** 一个 Transformer 语言模型（仅解码器结构）：堆叠多个 Transformer 块，将输入词元 $w_i$ 映射为预测的下一个词元 $w_{i+1}$。](/images/speech-and-language-processing/slp-fig-8-15.png)
